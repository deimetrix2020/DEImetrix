...
<html lang="en">
            ...
            <div id="layoutSidenav_content">
            	<header class="page-header page-header-dark bg-gradient-primary-to-secondary pb-10">
                 ...
                 <div class="container mt-n10">
                    <!-- Area chart example-->
                    <div class="card mb-4">
                        <div class="card-header bg-secondary" style="color: #000000;font-weight: bolder;">Upload Invite Users</div>
                        <div class="row">
							<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10" style="display: flex;margin: 20px;">
								...
								<a href="./invite_controller/downloadformat" target="_blank" style="font-size: 20px;margin-top: 7px;margin-left: 20px;" title="Download the template">
	                        ...
	                        <div class="card-body">
	                        	<div class="row">
		                        	<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
										<label class="cus-label">First Name</label>
			                            <div class="input-group mb-3">
										  <div class="input-group-prepend">
										    <span class="input-group-text" id="basic-addon1"><i class="fa fa-terminal" aria-hidden="true"></i></span>
										  </div>
										  <input type="text" id="firstname" name="firstname" class="form-control validate[required]" autocomplete="off" placeholder="First Name" aria-describedby="basic-addon1">
										</div>
									</div>
									<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
										<label class="cus-label">Last Name</label>
			                            <div class="input-group mb-3">
										  <div class="input-group-prepend">
										    <span class="input-group-text" id="basic-addon1"><i class="fa fa-terminal" aria-hidden="true"></i></span>
										  </div>
										  <input type="text" id="lastname" name="lastname" class="form-control validate[required]" autocomplete="off" placeholder="Last Name" aria-describedby="basic-addon1">
										</div>
									</div>
									<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
			                            ...
			                            <div class="input-group mb-3">
										  ...
										  <input type="text" id="emailid" name="emailid" class="form-control validate[required,custom[email]]" autocomplete="off" placeholder="Email" aria-describedby="basic-addon1">
										</div>
									</div>
									<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6" align="left" style="margin-top: 27px;">
										<button id="add" class="btn btn-md" type="button" onclick="inviteformsubmit()" style="font-weight: bolder;background-color: #000000;color: #d59d2b;">
							...
							<table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
								<thead style="font-size: 13px;">
									<tr class="bg-secondary" style="color: #000000;font-weight: bolder;text-align: center;">
						            	...
						            	<th class="labelsize">Email Sent Status</th>
						            	<th class="labelsize">Register Status</th>
						            	<th class="labelsize">Survey Status</th>
								...
								<tbody class="userdatatable">
						        	<c:forEach items="${requestScope.invite_list}" var="invitelist" varStatus="counter">
							            <tr>
							            	<td class="labelsize" style="text-align: center;">${counter.count}</td>
							            	<td class="labelsize">${invitelist.sFirstName}</td>
							            	<td class="labelsize">${invitelist.sLastName}</td>
							            	<td class="labelsize">${invitelist.sEmailId}</td>
							            	<c:choose>
								            	<c:when test="${fn:contains(invitelist.cIsEmailStatus,'Y')}">
							            	...
							            	<td class="labelsize">${invitelist.reg_status.sStatusName}</td>
							            	<td class="labelsize">${invitelist.survey_status.sStatusName}</td>
							            	<td class="labelsize">
							            		<fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${invitelist.dCreatedDate}" pattern='MM-dd-yyyy HH:mm:ss'/>
							            	</td>
							            	<td class="labelsize">${invitelist.sCreatedBy}</td>
							            	<td class="labelsize">
							            		<fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${invitelist.dUpdatedDate}" pattern='MM-dd-yyyy HH:mm:ss'/>
							            	</td>
							            	<td class="labelsize">${invitelist.sUpdatedBy}</td>
							            	<td class="labelsize" align="center">
												<a onclick="deleteinvitefrm('${invitelist.id}', 'soft')" href="#" title="Delete" style="color:#FF0000;margin-left: 10px;">
    ...
    <script type="text/javascript">
		...
		function inviteformsubmit()
		{
			...
			var demo_form = $("#inviteform").serialize();
			$("#inviteform :input").prop("disabled", true);
			$.ajax({
				type:'POST',
				url: "./invite_controller/createRecord",	
				data: demo_form,
		...
		function uploadCSVFile()
		{
			...
			if(fileExtension == "csv" || fileExtension == "CSV")
			{
				...
				if(typeof FormData == "undefined")
				{
					...
					xhr.open("POST", './invite_controller/uploadInviteUsersCSV');
					xhr.send(filetrans);
				} else
				{
				...
				xhr.onloadend = function (data) {
					...
					{
						setTimeout(function() {window.open("./invite_controller/faileddata");}, 700);
					}
					//$('#userform').validationEngine('hide');
					setTimeout(function() {
				         $("#result_label").fadeOut(100);
				         location.reload();
				      }, 5000);
					$(".uploadfile_btn").attr('disabled', false);
				};
		...
		function deleteinvitefrm(id, key)
		{
			$.confirm({
				title: 'Delete Record',
		...
		function deleterecord(id, key)
		{
			$.ajax({
				type:'POST',
				url: "./invite_controller/deleteRecord",	
				data: {"invite_id" : id, "key" : key},
				success: function(data)
				{
					if(data)
					{
